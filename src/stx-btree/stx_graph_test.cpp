...
//typedef std::pair<KEY_TYPE, PAYLOAD_TYPE> value_type;
#define value_type std::pair<KEY_TYPE, PAYLOAD_TYPE>
...
std::string BASE_FILENAME, DYNAMIC_FILENAME;
...
tlx::BTree<KEY_TYPE, value_type, key_of_value> alex_graph;

#ifdef SANITY_TEST
std::vector<std::vector<int32_t>> sanity_graph;
#endif
  ...
  while ((o = getopt_long(argc, argv, "b:d:j:o:t:v:s:x:y:h", longopts, &index)) != -1) {
    switch (o) {
      case 'v':
        ...
        std::cout << "Global vcount = " << global_vcount << std::endl;
        break;
      case 'h':
        print_usage();
        exit(0);
      ...
      case 'b':
        ...
        std::cout << "first input file = " << BASE_FILENAME << std::endl;
        break;
      case 'x':
        sscanf(optarg, "%lld", &num_base_edges);
        std::cout << "number of entries in the first input file = " << num_base_edges << std::endl;
        break;
      case 'd':
        DYNAMIC_FILENAME = optarg;
        std::cout << "second input file = " << DYNAMIC_FILENAME << std::endl;
        break;
      case 'y':
        sscanf(optarg, "%lld", &num_dynamic_edges);
        std::cout << "number of entries in the second input file = " << num_dynamic_edges << std::endl;
        break;
      case 'j':
        job = atoi(optarg);
        break;
...
//  alex_graph.bulk_load(base_values, num_edges);
  for (int64_t i = 0; i < num_base_edges; i++) {
    alex_graph.insert(value_type(base_values[i].first, base_values[i].second));
//    num_edges += 1;
  }
  end = mywtime();
  std::cout << "base graph bulk loaded into ALEX" << std::endl;
  std::cout << "B-graph load time = " << (end - start) << std::endl;
...
//    if(i && i%1000000 == 0) std::cout << "dynamic edge inserted: " << i << std::endl;
    alex_graph.insert(value_type(dynamic_values[i].first, dynamic_values[i].second));
    num_edges += 1;
  }
  end = mywtime();
  std::cout << "D-graph load time = " << (end - start) << std::endl;

  // inserting a special key for the last vertex
  alex_graph.insert(value_type(PUT_KEY(global_vcount, 0), -1));
//  alex_graph.insert(value_type(PUT_KEY(global_vcount, 0), make_pair(num_edges, -1)));
  num_edges += 1;
  free(dynamic_values);
...
void check_sanity() {
  ...
  std::cout << "Congratulation! Sanity check have been completed without any error!" << std::endl;
}
...
//      weight_sum += it.payload().second;
  ...
  std::cout << "Whole graph access time = " << (end - start) << std::endl;
}
...
void PrintTopPRScores(const pvector<ScoreT> &scores) {
  std::vector<std::pair<VERTEX_TYPE, ScoreT>> score_pairs(global_vcount);
  for (VERTEX_TYPE n=0; n < global_vcount; n++) {
    score_pairs[n] = std::make_pair(n, scores[n]);
  }
  int k = 5;
  std::vector<std::pair<ScoreT, VERTEX_TYPE>> top_k = TopK(score_pairs, k);
...
#pragma omp parallel
  {
    std::vector<std::vector<VERTEX_TYPE> > local_bins(0);
    size_t iter = 0;
...
//            WEIGHT_TYPE new_dist = dist[u] + it.payload();
            WEIGHT_TYPE new_dist = dist[u] + it->second;
